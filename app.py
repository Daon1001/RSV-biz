...
def innobiz_assessment_page():
    ...
    with col_ai:
        if st.button("🤖 AI 컨설팅 리포트 생성", type="secondary", use_container_width=True, key="ib_ai"):
            with st.spinner("Claude가 분석 중... (10~20초)"):
...
def dashboard_page():
    st.title("🏆 메인비즈·이노비즈 AI 마스터 컨설턴트")
    st.caption("컨설턴트 내부용 통합 관리 시스템 v2.1")

    companies_db = gist_load(_companies_filename(), get_default_companies_db)
    companies = companies_db.get("companies", {})

    # ── 메인비즈 통계 ──
    st.subheader("🏆 메인비즈 현황")
    col1, col2, col3, col4 = st.columns(4)
